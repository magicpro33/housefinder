...
st.caption(
    "Search houses by ZIP code, age, and estimated value. "
    "Stored ZIP data is used without an API call."
)

cached_zips = list_cached_zips()
cached_by_zip = {item.zip_code: item for item in cached_zips}
...
with st.sidebar:
    ...
    source_mode = st.radio(
        ...
        help="Stored data is committed under data/cache and does not need an API key.",
    )
    if source_mode == "Stored ZIP data":
        if cached_zips:
            selected_zip = st.selectbox(
                "Stored ZIP code",
                options=list(cached_by_zip),
                format_func=lambda zip_code: f"{zip_code} — {cached_by_zip[zip_code].summary}",
            )
        else:
            selected_zip = ""
            st.warning("No stored ZIP files were found in data/cache.")
        zip_code = st.text_input("ZIP code", value=selected_zip, max_chars=5)
        refresh_from_api = False
    else:
        zip_code = st.text_input("ZIP code", value="29209", max_chars=5)
        refresh_from_api = st.checkbox(
            "Refresh saved ZIP from RentCast",
            help="Uses the RentCast API and replaces the runtime cache.",
        )
    ...
    submitted = st.button("Search homes", type="primary", use_container_width=True)

    st.divider()
    st.caption(f"{len(cached_zips)} ZIP data dump(s) bundled with this app")
    if source_mode == "RentCast API":
        st.caption(
            "On Streamlit Cloud, API-fetched cache updates are temporary. "
            "Commit data/cache files to GitHub to make them permanent."
        )

if submitted:
    try:
        with st.spinner("Loading property records…"):
            _run_search(
                zip_code.strip(),
                age_range[0],
                age_range[1],
...
if "search_results" not in st.session_state:
    st.info("Choose a stored ZIP code and select **Search homes**.")
    st.stop()
